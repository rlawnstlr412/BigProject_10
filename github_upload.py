...
# -------------------------------------------------------------전체 견인 통계-------------------------------------------------------------
    if selected == "데이터":
        ...
        with st.sidebar:
            st.header('날짜 구간을 선택하세요')
            slider_date = st.slider('', min_value=datetime(2022, 1, 1), max_value=datetime(2022, 2, 28), value=(datetime(2022, 1, 1), datetime(2022, 2, 28)), format='YY/MM.DD')
            
            
        col1, col2 = st.columns([0.6, 0.4])
        with col1:
            st.header('전체 견인 통계') 
            line_df = pd.DataFrame({'months': pd.date_range('2018-01-01', periods=12, freq='M'),
                           ' ': [7552, 8080, 7707, 7718, 13895, 14423, 14375,
...
# -------------------------------------------------------------TOP3통계-------------------------------------------------------------        
        with col2:
            ...
            #mark_bar(size= )에서 바의 요소의 너비 결정, color에서 컬러 결정
            bar = alt.Chart(bar_df).mark_bar(size=50, color="#FFAA00").encode(
                #x축 y축의 이름이 데이터의 컬럼명과 같아야함.
                x='업체명',
                y=' ',
            ).properties(
...
# streamlit run Desktop\BP\github_upload
