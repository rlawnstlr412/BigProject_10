...
# CCTV geojson 구 별 분리
gdf = geopandas.read_file("Desktop/Final/cctv.geojson")
...
cctv_info = pd.read_csv('Desktop/Final/cctv_info.csv', encoding='cp949')

songpa_cctv = cctv_info.head(7)
songpa1_cctv = cctv_info.head(5)
gangnam_cctv = cctv_info.tail(6)
gangnam1_cctv = cctv_info.tail(5)
data_vis = pd.read_csv('Desktop/Final/data_vis.csv', encoding='cp949')
...
def show_map(lat, lon, zoom, data):
    ...
    df = pd.read_csv("Desktop/Final/seoul_population.csv")
...
def show_map_grey100(lat, lon, zoom, data):
    ...
    folium.LayerControl().add_to(m)
    folium.CircleMarker([37.5877, 126.94],  radius='100', fill=True, fill_color='red',color='red').add_to(m)
    folium.CircleMarker([37.6077, 127.04],  radius='100', fill=True, fill_color='red',color='red').add_to(m)
    folium.CircleMarker([37.4877, 126.94],  radius='100', fill=True, fill_color='red',color='red').add_to(m)
    folium.CircleMarker([37.4877, 127.004], radius='100', fill=True, fill_color='red',color='red').add_to(m)
    folium.CircleMarker([37.4977, 127.114], radius='100', fill=True, fill_color='red',color='red').add_to(m)
    folium.CircleMarker([37.5377, 127.044], radius='100', fill=True, fill_color='red',color='red').add_to(m)
    linear.add_to(m)
    st_data = st_folium(m, width=1000, height=500)

def show_map_grey300(lat, lon, zoom, data):

    df = pd.read_csv("dD/seoul_population.csv")
    ...
    folium.LayerControl().add_to(m)
    folium.CircleMarker([37.5877, 126.94],  radius='150', fill=True, fill_color='red',color='red').add_to(m)
    folium.CircleMarker([37.6077, 127.04],  radius='150', fill=True, fill_color='red',color='red').add_to(m)
    folium.CircleMarker([37.4877, 126.94], radius='150',fill=True, fill_color='red',color='red').add_to(m)
    folium.CircleMarker([37.4877, 127.004], radius='150',fill=True, fill_color='red',color='red').add_to(m)
    folium.CircleMarker([37.4977, 127.114], radius='150',fill=True, fill_color='red',color='red').add_to(m)
    folium.CircleMarker([37.5377, 127.044], radius='150',fill=True, fill_color='red',color='red').add_to(m)
    linear.add_to(m)
    st_data = st_folium(m, width=1000, height=500)
...
# -------------------------------------------------------------[전체] 관제 설정-------------------------------------------------------------
    if selected == "관제":
        ...
        with st.sidebar:
            ...
            st.markdown('<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>', unsafe_allow_html=True)
            st.image(boda_logo["image_url"])
...
#                 x=alt.X('지역명명', axis=alt.Axis(labelAngle=0)),
#                 y=' ',
#             ).properties(
...
#             )
            ...
            bar = alt.Chart(bar_df).mark_bar(size=50, color="#2E9AFE").encode(
                #x축 y축의 이름이 데이터의 컬럼명과 같아야함.
                x=alt.X('지역명', axis=alt.Axis(labelAngle=0)),
                y=' ',
            ).properties(
                #width= 에서 바 차트의 너비 결정
                width=500,
...
# ------------------------------------------------------------[전체]견인구역 설정-------------------------------------------------------------
        ...
        with col3:
            st.header('PM 브랜드별 견인 건수') 
            #바 형태 차트 만들기
            bar_df = pd.DataFrame({
                '브랜드명': ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I'],
            ...
            bar = alt.Chart(bar_df).mark_bar(size=50, color="#2E9AFE").encode(
                #x축 y축의 이름이 데이터의 컬럼명과 같아야함.
                x=alt.X('브랜드명', axis=alt.Axis(labelAngle=0)),
                y=' ',
            ).properties(
                #width= 에서 바 차트의 너비 결정
                width=500,
        ...
        with col4:
            ...
            # 범례 추가
            ax1.legend(labels, title="견인구역", loc="upper left", bbox_to_anchor=(1, 0, 0.5, 1))
...
# ------------------------------------------------------------[전체]주차장 설정-------------------------------------------------------------
        ...
        with col7:
            ...
            slider_value = st.slider('',  min_value=0, max_value=5, value=0, step=1, label_visibility="collapsed")

            st.subheader('설치기준 반경')
            slider_radius = st.slider('',  min_value=0, max_value=500, value=0, step=100, label_visibility="collapsed")
            st.markdown('#')
        ...
        with col8:           
            ...
            st.header('PM 주차장 현황')
            if slider_radius == 0:
                show_map_grey(37.56,127.07,11,geo_json_data)
            elif slider_radius <= 200:
                show_map_grey100(37.56,127.07,11,geo_json_data)
            else:
                show_map_grey300(37.56,127.07,11,geo_json_data)
...
# -------------------------------------------------------------[강남구] 관제 설정-------------------------------------------------------------
    if selected == "관제":
        ...
        with col4:
            st.header('CCTV')
            st.video('Desktop/Final/subway_exit.mp4')
...
# -------------------------------------------------------------사이드바 설정(사진)-------------------------------------------------------------
# Content for logos
seoul_logo = {"image_url": "Desktop/Final/seoul_img.png"}
boda_logo = {"image_url": "Desktop/Final/보다.png"}

# Streamlit app
with st.sidebar:
    # Other sidebar content
    st.image(seoul_logo["image_url"])
    # Center-aligning the content in the sidebar
    st.markdown("""
        <style>
...
#         st.header('연락처 010-0000-0000')
#         st.markdown('<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>', unsafe_allow_html=True)
#         st.image(boda_logo["image_url"], width=150)
